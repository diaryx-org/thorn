//! `svg-editor` — the profile on the command line. `check` is the test the
...
use svg_editor_core::Drawing;

#[derive(Parser)]
#[command(
    name = "svg-editor",
    version,
    about = "A drawing editor over twig's SVG"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
        ...
        Command::Check { file } => {
            ...
            if findings.is_empty() {
                println!(
                    "{}: conforms to profile {}",
                    file.display(),
                    svg_editor_core::profile::VERSION
                );
                Ok(ExitCode::SUCCESS)
            } else {
                Ok(ExitCode::FAILURE)
