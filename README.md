...
| [`packages/thorn-swift`](packages/thorn-swift) | **The Swift package.** `ThornFFI` is the committed generated binding; `Thorn` is `DrawingDocument` (the gestures with Foundation types at the edges), `CanvasModel` (the canvas with no view in it: tool, selection, the drag in flight, the zoom and pan — a pinch's, a scroll's, ⌘+/⌘−/⌘0's — and drawing into a `CGContext` through resvg-swift: the page as a sheet on a desk in the view's light or dark appearance, the file's `currentColor` ink following it, the picture unclipped, so a shape dragged off the edge is seen taking the page with it, and ⌘0 fits the page as it stands), `DrawingCanvasView` (AppKit / UIKit) and `DrawingEditor` (SwiftUI, with the toolbar). `Toolset.swift` is the toolbar's shape, Excalidraw's: lock, hand, select, rectangle, diamond, ellipse, arrow, line, draw, text, note, eraser, each with its keys (`R` or `2`, `Q` for the lock, Escape for select); the layering and grouping commands are a menu. Every tool has its gesture in the core now. `Package.swift` sits at the repo root because SwiftPM needs it there. |
